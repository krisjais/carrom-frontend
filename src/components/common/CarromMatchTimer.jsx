...
export function CarromMatchTimer({
  ...
  // Arena Stadium Variant (For Live Broadcast Hero Board - Compact)
  if (variant === 'arena') {
    return (
      <div className="flex flex-col items-center justify-center space-y-2 w-full max-w-xs sm:max-w-sm mx-auto p-2.5 sm:p-3 rounded-xl bg-white/90 backdrop-blur-md border border-[#D5C4A1] shadow-sm text-[#2B1B10]">
        <div className="flex items-center justify-between w-full text-[10px] font-mono border-b border-[#DFCBB5] pb-1.5">
          <div className="flex items-center gap-1.5 text-[#5A3E28] font-bold uppercase tracking-wider">
            <Clock className={`w-3 h-3 ${timerStats.isExpired || timerStats.isUrgent ? 'text-[#E74C3C] animate-pulse' : 'text-[#7A614A]'}`} />
            <span>Round Timer · {timerStats.baseMinutes}m Cap</span>
          </div>
          <span className={`px-2 py-0.5 rounded-full font-bold uppercase text-[9px] tracking-wider ${
            timerStats.isExpired
              ? 'bg-red-100 text-[#E74C3C] border border-red-300 animate-pulse'
              : timerStats.isPaused
            ...
            {timerStats.isExpired ? '⚠️ Completed' : timerStats.isPaused ? '⏸ Paused' : '● Live In Play'}
          </span>
        </div>

        {/* Stadium Digits - Compact */}
        <div className="flex items-baseline justify-center gap-1.5 py-0.5">
          <span
            className={`text-3xl sm:text-4xl font-mono font-black tabular-nums tracking-tight filter drop-shadow-2xs ${
              timerStats.isExpired
          ...
          <span className="text-[10px] font-mono font-bold uppercase text-[#7A6753] tracking-wider">
            {timerStats.isExpired ? 'Ended' : 'Remaining'}
          </span>
        </div>

        {/* Progress Track / Animated Stick */}
        <div className="w-full bg-[#EADBCE] h-2 sm:h-2.5 rounded-full overflow-hidden">
          <div
        ...
        {/* Context Note */}
        <div className="text-[9px] font-mono text-[#7A6753] flex items-center justify-between w-full pt-0.5">
          <span>Main Board 01</span>
          <span>{timerStats.isExpired ? 'Match Time Completed' : 'Sequential Match Flow'}</span>
        </div>
      </div>
    );
  }
