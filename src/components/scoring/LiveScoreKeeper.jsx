...
export const LiveScoreKeeper = ({ match, onUpdate }) => {
  ...
  // Stop Live Match (Revert back to scheduled queue)
  const handleStopLive = async () => {
    setLoading(true);
    setMessage(null);
    try {
      const res = await api.stopLiveMatch(match._id);
      if (res.success) {
        setIsStopLiveModalOpen(false);
        setMessage({
          type: 'success',
          text: `Match #${match.matchNumber} live status stopped. Reverted to Scheduled queue.`
        });
        if (onUpdate) onUpdate(res.match);
      }
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Failed to stop live match.' });
      setIsStopLiveModalOpen(false);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6 max-w-4xl mx-auto text-[#4A4238] dark:text-[#F5F1E8] transition-colors duration-200">
            ...
            {match.status === 'live' ? (
              <>
                ...
                <button
                  ...
                  disabled={loading}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#FDEDEC] dark:bg-[#E74C3C]/15 hover:bg-[#FADBD8] dark:hover:bg-[#E74C3C]/25 text-[#E74C3C] border border-[#E74C3C]/30 text-xs font-mono font-bold transition-all cursor-pointer disabled:opacity-50"
      ...
      {!isMatchCompleted ? (
        ...
        <div className="editorial-card rounded-2xl p-6 sm:p-8 border border-[#E8E1D5] dark:border-[#2B3034] bg-white dark:bg-[#15191C] space-y-6 shadow-xs text-center">
          ...
          {/* Secondary Action: Stop Live Button if started mistakenly */}
          {match.status === 'live' && (
            <div className="pt-5 border-t border-[#E8E1D5] dark:border-[#2B3034] flex flex-wrap items-center justify-between gap-3 text-left">
              <div>
                <span className="text-xs font-serif font-bold text-[#3E342B] dark:text-[#F5F1E8] block">
                  Started this match mistakenly?
                </span>
                <p className="text-[11px] text-[#7E7060] dark:text-[#817B72] font-mono">
                  Revert match back to scheduled queue and free up the Main Carrom Board.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsStopLiveModalOpen(true)}
                disabled={loading}
                className="px-4 py-2 rounded-xl bg-white dark:bg-[#181C1F] hover:bg-[#FDEDEC] dark:hover:bg-[#E74C3C]/15 text-[#E74C3C] border border-[#E74C3C]/30 text-xs font-mono font-bold flex items-center gap-1.5 transition-all cursor-pointer disabled:opacity-50 shrink-0"
              >
                <Square className="w-3.5 h-3.5 fill-current" />
                <span>STOP LIVE MATCH</span>
              </button>
            </div>
          )}
        </div>
      ...
      {/* --- STOP LIVE CONFIRMATION MODAL --- */}
      <Modal
        isOpen={isStopLiveModalOpen}
        onClose={() => {
          if (!loading) setIsStopLiveModalOpen(false);
        }}
        title="Stop Live Match"
        maxWidth="max-w-md"
      >
        <div className="space-y-5 py-1 text-[#4A4238] dark:text-[#F5F1E8]">
          <div className="flex items-center gap-4 p-4 rounded-xl bg-[#FDEDEC] dark:bg-[#E74C3C]/15 border border-[#E74C3C]/30">
            <div className="w-10 h-10 rounded-full bg-white dark:bg-[#15191C] text-[#E74C3C] flex items-center justify-center shrink-0 border border-[#E74C3C]/30">
              <AlertTriangle className="w-5 h-5" />
            </div>
            <div className="text-left">
              <h4 className="text-sm font-serif font-bold text-[#3E342B] dark:text-[#F5F1E8]">
                Revert Match to Scheduled?
              </h4>
          ...
          <p className="text-xs text-[#7E7060] dark:text-[#B8B1A5] leading-relaxed text-left">
            If you started this match by mistake, clicking confirm will remove it from the Main Carrom Board and revert its status back to <strong>Scheduled</strong>. The arena board will immediately become free for other matches.
          </p>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-[#E8E1D5] dark:border-[#2B3034]">
            <button
              ...
              disabled={loading}
              className="px-4 py-2 rounded-xl bg-white dark:bg-[#181C1F] hover:bg-[#FAF9F6] border border-[#D5C4A1] dark:border-[#2B3034] text-xs font-mono font-bold cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              disabled={loading}
              className="px-5 py-2 rounded-xl bg-[#E74C3C] hover:bg-[#C0392B] text-white text-xs font-mono font-bold flex items-center gap-1.5 cursor-pointer shadow-xs disabled:opacity-50"
            >
              <Square className="w-3.5 h-3.5 fill-current" />
              <span>{loading ? 'Stopping...' : 'Confirm Stop Live'}</span>
            </button>
          </div>
        </div>
      </Modal>
