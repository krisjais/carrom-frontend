...
import { Activity, Crown, Clock, RefreshCw, Trophy, CheckCircle2, Play, Timer, Sparkles } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-3">
        <div className="flex items-center justify-center gap-2 mb-1">
          <MainBoardBadge />
          <span className="text-[11px] font-mono text-[#FFBA00] font-black uppercase tracking-widest">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold font-display text-white flex items-center gap-2.5">
            <span className={`w-3 h-3 rounded-full ${currentMatch ? 'bg-emerald-400 animate-ping' : 'bg-purple-700'}`} />
            <span>CURRENT MATCH (LIVE)</span>
          </h2>
          <span className="text-xs text-[#D8C7F0] flex items-center gap-1.5 font-mono">
        ...
        {currentMatch ? (
          <div className="glass-card p-6 sm:p-8 space-y-6 border-2 border-emerald-500/50 bg-gradient-to-br from-[#2C0854] to-[#17022E] shadow-2xl">
            {/* Top line */}
            <div className="flex items-center justify-between pb-4 border-b border-[#4A138C] text-xs">
              <div className="flex items-center gap-2.5">
                <MainBoardBadge />
                <CategoryBadge category={currentMatch.category} />
                <span className="font-mono text-[#FFBA00] font-bold">{currentMatch.roundName}</span>
              </div>
            ...
            <div className="grid grid-cols-3 items-center text-center gap-4 py-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-[#D8C7F0] uppercase font-bold mt-1.5 block font-mono">Single-Game Knockout</span>
              </div>
            ...
            <div className="pt-4 border-t border-[#4A138C] flex items-center justify-between text-xs text-[#D8C7F0]">
              <span className="font-mono text-[#FFBA00]">Main Carrom Board Arena</span>
          ...
          <div className="p-12 text-center sport-card space-y-2">
            <Activity className="w-8 h-8 text-[#FDB095] mx-auto opacity-70" />
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-[#D8C7F0] font-mono">{readyQueue.length} In Line</span>
        </div>

        {readyQueue.length === 0 ? (
          <div className="p-8 text-center sport-card text-xs text-[#D8C7F0]">
              ...
              >
                <div className="flex items-center justify-between text-xs pb-2 border-b border-[#4A138C]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[#D8C7F0]">{match.roundName}</span>
                  </div>
      ...
      {completedMatches.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/results" className="text-xs font-bold text-[#FDB095] hover:text-[#FFBA00]">
              View All Results →
            </Link>
          </div>
